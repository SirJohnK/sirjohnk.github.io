...
```
**BUT** when we need to access a `Resource` that is defined with `OnPlatform`, `OnIdiom` and/or references a `DynamicResource` in a `Merged Dictionary`, it gets trickier!
Especially if this is nested in multiple layers!

Let's look at an example:
```c#
<Application.Resources>
    <ResourceDictionary>
        <OnPlatform x:Key="SmallCornerRadius" x:TypeArguments="CornerRadius">
            <On Platform="Android" Value="{DynamicResource}"/>
            <On Platform="iOS"  Value="{DynamicResource PrimaryCornerRadius}/>
        </OnPlatform>
    </ResourceDictionary>
</Application.Resources>
```
